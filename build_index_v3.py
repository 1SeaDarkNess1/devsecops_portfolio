...
<body>
    ...
    <script>
        ...
        async function fetchSbom() {
            try {
                const res = await fetch('/static/sbom.json');
                if(res.ok) {
                    const data = await res.json();
                    if(data.artifacts) {
                        globalSbom = data.artifacts;
                        document.getElementById('sbom-count').innerText = `${globalSbom.length} components`;
        ...
        function renderSbom(arr) {
            ...
            arr.slice(0, 30).forEach(a => { // show max 30
                const lic = a.licenses && a.licenses[0] ? a.licenses[0] : 'UNKNOWN';
        ...
        document.getElementById('sbom-search').addEventListener('input', (e) => {
            ...
            sbomT = setTimeout(() => {
                ...
                if(term === '') renderSbom(globalSbom);
                    ...
                    const matched = globalSbom.filter(x => x.name.toLowerCase().includes(term) || (x.licenses&&x.licenses[0]&&x.licenses[0].toLowerCase().includes(term)));
                    renderSbom(matched);
                }
            }, 150);
        });
        ...
        async function initGlobe() {
            ...
            // Fetch Mocked Threats targeting FRA
            let threats = [];
            try {
                const tr = await fetch('/static/threats.json');
                if(tr.ok) { 
                    threats = await tr.json(); 
                    let c = 0; threats.forEach(x => c += x.count);
                    document.getElementById('threat-counter').innerText = c.toLocaleString() + '+';
                }
            } catch(e){}
